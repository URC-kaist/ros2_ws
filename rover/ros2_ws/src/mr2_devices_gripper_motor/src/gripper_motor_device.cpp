...
namespace mr2_devices_gripper_motor {

class GripperMotorDevice : public CanDevice {
public:
  void configure(const hardware_interface::ComponentInfo &info,
                 rclcpp::Node *node) override {
    node_ = node;
    logger_ = node_ ? node_->get_logger()
                    : rclcpp::get_logger("gripper_motor_device");
    ...
    const uint32_t position_id = CanProtocol::gripper_position_id(node_id_);
  ...
  void process(const rclcpp::Time &) override {
    ...
    const auto frame =
        CanProtocol::encode_gripper_command(node_id_, centi_deg);
    send(frame, bus_);
  }

  void export_state(double *&position, double *&velocity,
                    double *&effort) override {
    // Keep ROS state finite so TF remains valid before feedback arrives.
  ...
  void export_command(double *&command) override { command = &desired_command_rad_; }
...
private:
  ...
  void on_position(const can_frame &frame) {
    int16_t centi_deg = 0;
    if (!CanProtocol::decode_gripper_position(frame, node_id_, centi_deg)) {
      return;
    }
  ...
  rclcpp::Logger logger_{rclcpp::get_logger("gripper_motor_device")};
  std::string iface_;
  uint8_t node_id_{0};

  // Start neutral so robot_state_publisher remains well-formed before feedback.
...
PLUGINLIB_EXPORT_CLASS(mr2_devices_gripper_motor::GripperMotorDevice,
                       CanDevice)
