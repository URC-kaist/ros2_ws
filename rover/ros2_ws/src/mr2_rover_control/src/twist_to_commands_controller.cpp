...
TwistToCommandsController::update(const rclcpp::Time &,
  ...
  const bool running =
      (mission_state_.has_value() &&
       mission_state_.value() == kMissionStateRunning);
  ...
  if (running) {
    ...
    solver_->setErrorAlpha(solver_error_alpha_);
    ...
    if (wheel_radius_ <= 0.0) {
      RCLCPP_ERROR_THROTTLE(
          get_node()->get_logger(), *get_node()->get_clock(), 2000,
          "wheel_radius must be > 0 (got %.3f); zeroing commands",
          wheel_radius_);
      publishZeros();
    } else {
    ...
    // Keep solver's internal filter aligned with actual steering to avoid
    // stale state when switching back to the running command stream.
    if (solver_) {
      FourWheelSteeringSolver::Cmd sync_cmd{0.0, 0.0, 0.0};
      (void)solver_->solve(sync_cmd, current_steer);
    }
  }
