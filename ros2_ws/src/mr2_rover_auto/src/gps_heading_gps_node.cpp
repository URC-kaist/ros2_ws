...
public:
  GpsHeadingGpsNode() : Node("gps_heading_gps_node") {
    ...
    yaw_offset_rad_ = declare_parameter<double>(
        "yaw_offset_rad", -M_PI_2); // 90° left-mount (north-south = north)
    max_gps_age_ = declare_parameter<double>("max_gps_age",
                                             1.0); // [s] Ignore old GNSS yaw
    max_pair_skew_ = declare_parameter<double>(
        "max_pair_skew", 0.2); // [s] Threshold of simultaneity
    min_baseline_xy_ = declare_parameter<double>(
        "min_baseline_xy", 0.4); // [m] Minimum allowed distance between GPS RX
                                 // to reject huge error
    zero_alt_for_heading_ = declare_parameter<bool>(
        "zero_alt_for_heading",
        true); // True to ignore altitude difference / altitude noise; Earth is
               // too big for this
    baseline_direction_ = declare_parameter<int>(
        "baseline_direction", +1); // +1: (north - south), -1 flips

    auto qos = rclcpp::SensorDataQoS(); // QoS tuned for sensors

    gps_north_sub_ = create_subscription<sensor_msgs::msg::NavSatFix>(
        "/right_gnss/navsat", qos,
        std::bind(&GpsHeadingGpsNode::northCallback, this, _1));

    gps_south_sub_ = create_subscription<sensor_msgs::msg::NavSatFix>(
        "/left_gnss/navsat", qos,
    ...
    gps_odom_pub_ =
        create_publisher<nav_msgs::msg::Odometry>("/odometry/gps", qos);
  }
...
private:
  ...
  // Compute or store GNSS yaw if both north and south fixes are available and
  // healthy
  void tryUpdateGpsYaw() {
    // 0. Need both fixes and valid stamps
    if (!last_north_fix_ || !last_south_fix_)
      return;
    ...
    // 0.a Basic validity (finite lat/lon; optional: check status/covariance
    // type)
    if (!std::isfinite(N.latitude) || !std::isfinite(N.longitude) ||
        !std::isfinite(S.latitude) || !std::isfinite(S.longitude))
      return;

    // 0.b Enforce small pair skew (they should be nearly simultaneous)
    if (std::abs((tN - tS).seconds()) > max_pair_skew_)
      return;
    if (tN <= last_north_time_used_ || tS <= last_south_time_used_)
      return;

    // 1. Convert geodetic to ECEF (optionally zero altitude to reduce vertical
    // noise)
    ...
    //    NOTE: if your mounting uses the opposite direction, set
    //    baseline_direction_ = -1
    ...
    // 4. Calculate world yaw from baseline and apply mount offset (e.g., -π/2
    // for left-mount)
  ...
  Eigen::Vector3d ecefToENU(const Eigen::Vector3d &diff, double ref_lat_deg,
                            double ref_lon_deg) {
    double p = ref_lat_deg * M_PI / 180.0; // latitude (phi) in radian
    double l = ref_lon_deg * M_PI / 180.0; // longitude (lambda)
    Eigen::Matrix3d R;
    // https://gssc.esa.int/navipedia/index.php/Transformations_between_ECEF_and_ENU_coordinates
    R << -std::sin(l), std::cos(l), 0, -std::sin(p) * std::cos(l),
        -std::sin(p) * std::sin(l), std::cos(p), std::cos(p) * std::cos(l),
        std::cos(p) * std::sin(l), std::sin(p);
    return R * diff;
  }
};
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(
      std::make_shared<GpsHeadingGpsNode>()); // share pointer, start loop
  rclcpp::shutdown();
  return 0;
}
